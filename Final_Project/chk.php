...
<body>
	...
	<div id="cont">
		...
		$onoo = mysql_query('select OrderNO from order1 where RollNo = "'.$rno.'"') or die(mysql_error());
		$ono = mysql_result($onoo,0,0);
		for($j=1;$j<=24;$j++)
		{
			if(isset($i[$j]))
			{
		...
		echo '<br/>'.$total;
		$querytotal = 'update order1 set Total = '.$total.' where OrderNO = '.$ono;
		mysql_query($querytotal) or die(mysql_error());
		?>
		
	</div>	
	<div id="bg"><div><table cellpadding="0" cellspacing="0"><tr><td><img alt="" src="file:///D|/xampp/htdocs/xampp/projects/Ravi-OS v-2/home_includes/dept road.jpg" /></td></tr></table></div></div>
